...
def login():
    if request.method == 'POST':
        ...
        if 'username' in body and type(body['username']) == str \
            ...
            if not user:
                return jsonify({'message': 'Entered user does not exist or username is incorrect'}), 404
    ...
    if movie_id:
        try:
            ...
            comments = Comment.query.join(User, Comment.user_id == User.id)\
                ...
                .filter(Comment.movie_id == movie_id and Comment.approved)
...
def post_vote():
    ...
    if 'movie_id' in body and type(body['movie_id']) == int \
        ...
        if not movie:
            return jsonify({'message': 'Entered movie_id is does not exist'}), 404
...
def post_comment():
    ...
    if 'movie_id' in body and type(body['movie_id']) == int \
        ...
        movie = Movie.query.filter_by(id=body['movie_id']).first()
        if not movie:
            return jsonify({'message': 'Entered movie_id does not exist'}), 404
...
def approve_comment(comment_id):
    if comment_id:
        try:
            ...
            if not comment:
                return jsonify({'message': 'Entered comment_id does not exist'}), 404
